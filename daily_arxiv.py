...
def sort_papers(papers):
    # Sort by the date shown in each row (newest first), tie-broken by arxiv id.
    def date_key(item):
        key, value = item
        m = re.search(r'\d{4}-\d{2}-\d{2}', value or '')
        return (m.group() if m else '', key)
    return dict(sorted(papers.items(), key=date_key, reverse=True))
